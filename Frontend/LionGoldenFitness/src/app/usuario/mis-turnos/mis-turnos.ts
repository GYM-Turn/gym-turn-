...
export class MisTurnos implements OnInit {
  ...
  cargarMisTurnos(): void {
    this.inscripcionService
      .getInscripcionesByUsuario(this.userId) // 🔥 ya filtra estado=1
  ...
  cancelarTurno(turno: Turno): void {
    this.inscripcionService
      ...
      .subscribe((inscripciones) => {
        ...
        // 1️⃣ Cambiar estado a cancelado (estado = 2)
        this.inscripcionService.cancelarInscripcion(inscripcion.id).subscribe(() => {
          // 2️⃣ Liberar cupo
          ...
          this.turnoService.updateTurno(turno.id, turnoActualizado).subscribe(() => {
